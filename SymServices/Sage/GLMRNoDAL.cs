...
//////////////                sqlText = @"
//////////////SELECT DISTINCT Id, name FROM glmrnos WHERE TransactionType='BDE'
//////////////AND name NOT IN(
//////////////SELECT DISTINCT d.MRNo
//////////////FROM  GLBDERequisitionFormEs d
//////////////WHERE 1=1
//////////////AND d.Post=1 AND d.IsRejected = 0
//////////////)
//////////////
//////////////";
                sqlText = @"
SELECT DISTINCT MRNo Name FROM GLBDERequisitionFormAs d 
LEFT OUTER JOIN GLBDERequisitions m ON d.GLBDERequisitionId = m.Id
WHERE 1=1
AND m.Post=1 AND m.IsApprovedL4 = 1 AND d.IsRejected = 0
AND d.MRNo NOT IN(
SELECT DISTINCT d.MRNo
FROM  GLBDERequisitionFormEs d
WHERE 1=1
...
";
                ...
                if (branchId > 0)
                {
                    sqlText += @" and m.BranchId=@BranchId";
                }
        ...
        public List<GLMRNoVM> DropDown(string tType = "", int branchId = 0)
        {
            ...
            try
            {
                ...
                #region sql statement
                sqlText = @"
SELECT DISTINCT Id, name FROM glmrnos WHERE TransactionType='PettyCash'
AND name NOT IN(
select distinct MRNo from GLBDERequisitionFormAs where Post=1 AND IsRejected = 0
)
--and name not in(
--select distinct name from glmrnos where TransactionType=@tType
--and isactive=1
--)
";
...
Id
,Name
,case when  TransactionType='PettyCash' then 'Petty Cash Requisition'
...
";
        ...
        //==================Insert =================
        public string[] Insert(GLMRNoVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            retResults[5] = "InsertGLMRNo"; //Method Name
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            int transResult = 0;
            #endregion
            ...
            try
            {
                ...
                vm.Id = _cDal.NextId("GLMRNos", currConn, transaction);
                if (vm != null)
                {
                    sqlText = "  ";
                    sqlText += @" 
...
";
                    ...
                    if (transResult <= 0)
                    {
                        ...
                        throw new ArgumentNullException("Unexpected error to update GLMRNos.", "");
                    }
                }
                else
                {
                    retResults[1] = "This GLMRNo already used!";
                    throw new ArgumentNullException("Please Input GLMRNo Value", "");
                }
                #endregion Save
                #region Commit
                if (Vtransaction == null)
        ...
        //==================InsertList =================
        public string[] Insert(List<GLMRNoVM> VMs, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            try
            {
                ...
                int NextId = _cDal.NextId("GLMRNos", currConn, transaction);
...
";
                    ...
                    foreach (GLMRNoVM item in VMs)
                    {
                        ...
                        //////////    exist = _cDal.ExistCheck("GLMRNos", conditionFields, conditionValues, currConn, transaction);
                        //////////    if (exist)
                        //////////    {
                        //////////        retResults[1] = "MR No: " + item.Name + " already used!";
                        //////////        //return retResults;
                ...
                {
                    retResults[1] = "Please Input GLMRNo Value!";
                    throw new ArgumentNullException(retResults[1], "");
                }
                #endregion Save
                #region Commit
        ...
        //==================Update =================
        public string[] Update(GLMRNoVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            try
            {
                ...
                if (transaction == null) { transaction = currConn.BeginTransaction("UpdateToGLMRNo"); }
                #endregion open connection and transaction
                ...
                if (vm != null)
                {
                    ...
                    if (transResult <= 0)
                    {
                        // throw new ArgumentNullException("GLMRNo Update", vm.BranchId + " could not updated.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("GLMRNo Update", "Could not found any item.");
                }
                if (Vtransaction == null && transaction != null)
                {
                    transaction.Commit();
