...
{
    public class GLPCRequisitionJournalDAL
    {
        ...
        #region Methods
        //==================SelectAll =================
        public List<GLPCRequisitionJournalVM> SelectAll(int Id = 0, string[] conditionFields = null, string[] conditionValues = null, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            string sqlText = "";
            List<GLPCRequisitionJournalVM> VMs = new List<GLPCRequisitionJournalVM>();
            GLPCRequisitionJournalVM vm;
            #endregion
            try
            {
...
,br.Name BranchName
,pcrj.Code                            
,pcrj.TransactionDateTime             
,pcrj.TransactionType
,ISNULL(pcrj.Post, 0) Post
,pcrj.Remarks                         
,pcrj.IsActive                        
,pcrj.IsArchive                       
,pcrj.CreatedBy                       
...
FROM GLPCRequisitionJournals  pcrj
LEFT OUTER JOIN GLBranchs br ON pcrj.BranchId = br.id
WHERE  1=1  AND pcrj.IsArchive = 0
";


                if (Id > 0)
                {
                    sqlText += @" and pcrj.Id=@Id";
                }
                ...
                while (dr.Read())
                {
                    vm = new GLPCRequisitionJournalVM();
        ...
        //==================Insert =================
        public string[] Insert(GLPCRequisitionJournalVM vm, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Initializ
            string sqlText = "";
            int Id = 0;
            ...
            retResults[5] = "InsertGLPCRequisitionJournal"; //Method Name
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
            int transResult = 0;
            #endregion
            ...
            try
            {
                ...
                vm.Id = _cDal.NextId("GLPCRequisitionJournals", currConn, transaction);
                DateTime transDate = Convert.ToDateTime(Ordinary.StringToDate(vm.TransactionDateTime));
                vm.Code = codedal.NextCodeAcc("GLPCRequisitionJournals", vm.BranchId, transDate, vm.TransactionType, currConn, transaction);
                ...
                if (vm != null)
                {
                    ...
                    sqlText = "  ";
                    sqlText += @" 
INSERT INTO GLPCRequisitionJournals(
Id
,BranchId
,Code
,TransactionDateTime
...
";
                    ...
                    if (transResult <= 0)
                    {
                        ...
                        throw new ArgumentNullException("Unexpected error to update GLPCRequisitionJournals.", "");
                    }
                    #endregion SqlExecution
                }
                else
                {
                    retResults[1] = "This GLPCRequisitionJournal already used!";
                    throw new ArgumentNullException("Please Input GLPCRequisitionJournal Value", "");
                }
                #endregion Save
                #region Commit
                if (Vtransaction == null)
        ...
        ////==================CreateJournal =================
        public string[] CreateJournal(GLPCRequisitionJournalVM vm, string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            try
            {
                ...
                if (ids.Length >= 1)
                {
                    ...
                    GLPettyCashRequisitionDAL glFTDAL = new GLPettyCashRequisitionDAL();

                    GLPCRequisitionJournalVM ejvm = new GLPCRequisitionJournalVM();
                    string[] conditionFields = {"pc.MadeJournal"};
                    ...
                    string GLPCRequisitionJournalId = retResults[2];
                    ...
                    for (int i = 0; i < ids.Length - 1; i++)
                    {
                        ...
                        #region Exist Check
                        string[] conditionField = { "GLPettyCashRequisitionId" };
                        string[] conditionValue = { ids[i] };
                        var exist = _cDal.ExistCheck("GLPCRequisitionJournalDetails", conditionField, conditionValue, currConn
                            , transaction);
                        if (exist)
                        {
                        ...
                        sqlText = "";
                        sqlText += @"
                                    insert into GLPCRequisitionJournalDetails(BranchId,GLPCRequisitionJournalId,GLPettyCashRequisitionId)
                                    select branchid,@GLPCRequisitionJournalId,Id from GLPettyCashRequisitions
                                    where Id=@GLPettyCashRequisitionId
                                    
                                    update GLPettyCashRequisitions set MadeJournal=1
                                    where id=@GLPettyCashRequisitionId
                        ";
                        SqlCommand cmdInsert = new SqlCommand(sqlText, currConn, transaction);
                        cmdInsert.Parameters.AddWithValue("@GLPettyCashRequisitionId", ids[i]);
                        cmdInsert.Parameters.AddWithValue("@GLPCRequisitionJournalId", retResults[2]);
                        var exeRes = cmdInsert.ExecuteNonQuery();
                        transResult = Convert.ToInt32(exeRes);
                    }
                    //if (transResult > 0)
        ...
        //==================Delete =================
        public string[] Delete(GLPCRequisitionJournalVM vm, string[] ids, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            string[] retResults = new string[6];
            retResults[0] = "Fail";//Success or Fail
            ...
            retResults[3] = "sqlText"; //  SQL Query
            retResults[4] = "ex"; //catch ex
            retResults[5] = "DeleteGLPCRequisitionJournal"; //Method Name
            int transResult = 0;
            int countId = 0;
            string sqlText = "";
            SqlConnection currConn = null;
            ...
            try
            {
                ...
                if (ids.Length >= 1)
                {
                    ...
                    {
                        string retVal = "";
                        retVal = _cDal.SelectFieldValue("GLPCRequisitionJournals", "Post", "Id", ids[i].ToString(), currConn, transaction);
                    ...
                    for (int i = 0; i < ids.Length - 1; i++)
                    {
                        #region Update GLPCRequisitionJournalDetails
                        sqlText = "";
                        sqlText += " ";
                        sqlText += "DELETE GLPCRequisitionJournalDetails";
                        sqlText += " WHERE GLPCRequisitionJournalId=@Id";
                        ...
                        {
                            retResults[3] = sqlText;
                            throw new ArgumentNullException("Unexpected error to update GLPCRequisitionJournalDetails.", "");
                        }
                        sqlText = " ";
                        sqlText = "DELETE GLPCRequisitionJournals";
                        sqlText += " WHERE Id=@Id";
                        ...
                        #endregion Update GLPCRequisitionJournalDetails
                    }
                    retResults[2] = "";// Return Id
                    retResults[3] = sqlText; //  SQL Query
                    #region Commit
                    if (transResult <= 0)
                    {
                        throw new ArgumentNullException("GLPCRequisitionJournal Delete", vm.Id + " could not Delete.");
                    }
                    #endregion Commit
                ...
                {
                    throw new ArgumentNullException("GLPCRequisitionJournal Information Delete", "Could not found any item.");
                }
                if (iSTransSuccess == true)
                {
                    if (Vtransaction == null)
                ...
                {
                    retResults[1] = "Unexpected error to delete GLPCRequisitionJournal Information.";
                    throw new ArgumentNullException("", "");
                }
            }
            #region catch
        ...
        public string[] Post(string[] ids, ShampanIdentityVM auditVM, SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            ...
            //            List<GLPCRequisitionJournalVM> sJournalVMs = new List<GLPCRequisitionJournalVM>();
            //            GLPCRequisitionJournalVM sJournalVM = new GLPCRequisitionJournalVM();
            ...
            //            #region Post
            //            retResults = _cDal.FieldPost("GLPCRequisitionJournals", "Id", ids[i], currConn, transaction);
            //            if (retResults[0].ToLower() == "fail")
            //            {
            //                throw new ArgumentNullException("GLPCRequisitionJournals Post", ids[i] + " could not Post.");
            //            }
            //            #endregion Post
            ...
            //    else
            //    {
            //        throw new ArgumentNullException("GLPCRequisitionJournal Information Post", "Could not found any item.");
            //    }
            //}
            //#region catch
            //catch (Exception ex)
        ...
        public List<GLPCRequisitionJournalVM> SelectAllToPost(string Id = "" , SqlConnection VcurrConn = null, SqlTransaction Vtransaction = null)
        {
            #region Variables
            SqlConnection currConn = null;
            SqlTransaction transaction = null;
...
select br.ExpenseAccountId HeadId  ,sum(subtotal) Dr,0 Cr from GLPettyCashRequisitionDetails
left outer join Branchs br on br.Id=GLPettyCashRequisitionDetails.BranchId
where GLPettyCashRequisitionDetails.GLPettyCashRequisitionId in(
select sjd.GLPettyCashRequisitionId from GLPCRequisitionJournalDetails sjd
where sjd.GLPCRequisitionJournalId=@Id)
group by br.ExpenseAccountId
union all
select br.CashAccountId HeadId,0 Dr,sum(subtotal) Cr from GLPettyCashRequisitionDetails
left outer join Branchs br on br.Id=GLPettyCashRequisitionDetails.BranchId
where GLPettyCashRequisitionDetails.GLPettyCashRequisitionId in(
select sjd.GLPettyCashRequisitionId from GLPCRequisitionJournalDetails sjd
where sjd.GLPCRequisitionJournalId=@Id)
