...
{
    [ShampanAuthorize]
    public class MPLSelfBankInformationController : Controller
    {
        ShampanIdentity identity = null;

        MPLSelfBankInformationRepo _repo = null;

        public MPLSelfBankInformationController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new MPLSelfBankInformationRepo(identity);
            }
            catch
            {
        ...
        {
            identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            _repo = new MPLSelfBankInformationRepo(identity, Session);
            ...
            IEnumerable<MPLBankInformationVM> filteredData;
            ...
            Func<MPLBankInformationVM, string> orderingFunction = (c =>
        ...
        public ActionResult Create()
        {
            ...
            MPLBankInformationVM vm = new MPLBankInformationVM();
            vm.Operation = "add";
            vm.ActiveStatus = "Y";
            return View(vm);
        }
        [Authorize(Roles = "Admin")]
        [HttpPost]
        public ActionResult CreateEdit(MPLBankInformationVM vm)
        {
            identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            _repo = new MPLSelfBankInformationRepo(identity, Session);
            ...
            try
            {
                if (vm.Operation.ToLower() == "add")
                {
                    ...
                    result = _repo.InsertToMPLBankInformation(vm);
                ...
                else if (vm.Operation.ToLower() == "update")
                {
                    ...
                    result = _repo.UpdateMPLBankInformation(vm);
                    Session["result"] = result[0] + "~" + result[1];
                    return RedirectToAction("Edit", new { id = result[2] });
                }
                else
            ...
            catch (Exception ex)
            {
                ...
                FileLogger.Log("MPLSelfBankInformationController", "CreateEdit", ex.ToString());
                return View("Create", vm);
            }
        }
        ...
        {
            var vm = new MPLSelfBankInformationRepo(identity, Session).SelectAll(Convert.ToInt32(ItemId)).FirstOrDefault();
