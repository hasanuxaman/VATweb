...
{
    [ShampanAuthorize]
    public class MPLZoneProfileController : Controller
    {
        ShampanIdentity identity = null;

        MPLZoneProfileRepo _repo = null;

        public MPLZoneProfileController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new MPLZoneProfileRepo(identity);
            }
            catch
            {
        ...
        {
            identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            _repo = new MPLZoneProfileRepo(identity, Session);
            ...
            IEnumerable<MPLZoneProfileVM> filteredData;
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredData = getAllData.Where(c =>
                       isSearchable1 && c.ZoneCode.ToLower().Contains(param.sSearch.ToLower())
                    || isSearchable2 && c.ZoneName.ToLower().Contains(param.sSearch.ToLower())
            ...
            Func<MPLZoneProfileVM, string> orderingFunction = (c =>
                sortColumnIndex == 1 && isSortable_1 ? c.ZoneCode :
        ...
        public ActionResult Create()
        {
            ...
            MPLZoneProfileVM vm = new MPLZoneProfileVM();
            vm.Operation = "add";
            vm.ActiveStatus = "Y";
            return View(vm);
        }
        [Authorize(Roles = "Admin")]
        [HttpPost]
        public ActionResult CreateEdit(MPLZoneProfileVM vm)
        {
            identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            _repo = new MPLZoneProfileRepo(identity, Session);
            ...
            try
            {
                if (vm.Operation.ToLower() == "add")
                {
                    ...
                    result = _repo.InsertToMPLZoneProfile(vm);
                ...
                else if (vm.Operation.ToLower() == "update")
                {
                    ...
                    result = _repo.UpdateMPLZoneProfile(vm);
                    Session["result"] = result[0] + "~" + result[1];
                    return RedirectToAction("Edit", new { id = result[2] });
                }
                else
        ...
        {
            var vm = new MPLZoneProfileRepo(identity, Session).SelectAll(Convert.ToInt32(ItemId)).FirstOrDefault();
