    ...
    public class PurchaseController : Controller
    {
        ...
        public ActionResult _index(JQueryDataTableParamVM param, PurchaseMasterVM paramVM)
        {
            ...
            var result = from c in displayedCompanies
                ...
                , Convert.ToDateTime(c.ReceiveDate).ToString("dd-MMM-yyyy hh:mm tt")
        ...
        public ActionResult Create(string tType)
        {
            try
            {
                ...
                if (tType == "PurchaseCN" || tType == "PurchaseDN" || tType == "PurchaseReturn")
                {
                    ...
                    vm.TransactionType = tType;
                    vm.ReceiveDate = vm.ReceiveDate + DateTime.Now.ToString(" HH:mm");
        ...
        public ActionResult CreateEdit(PurchaseMasterVM vm)
        {
            ...
            try
            {
                ...
                vm.InvoiceDate = Convert.ToDateTime(vm.InvoiceDate).ToString("yyyy-MMM-dd") + DateTime.Now.ToString(" HH:mm:ss");
                vm.ReceiveDate = Convert.ToDateTime(vm.ReceiveDate).ToString("yyyy-MMM-dd HH:mm");
                                 //+ DateTime.Now.ToString(" HH:mm:ss");
                vm.RebateDate = Convert.ToDateTime(vm.RebateDate).ToString("yyyy-MMM-dd") + DateTime.Now.ToString(" HH:mm:ss");
                ...
                else if (vm.Operation.ToLower() == "update")
                {
                    ...
                    conditionFields = new string[] { "pih.PurchaseInvoiceNo" };
                    conditionValues = new string[] { vm.PurchaseInvoiceNo };
        ...
        public ActionResult Edit(string id, string TransactionType, decimal NetBill = 0, decimal TDSAmount = 0)
        {
            ...
            try
            {
                ...
                PurchaseTrakingVMs = _repo.SelectTrakingsDetail(PurchaseDetailVMs, vm.PurchaseInvoiceNo);
        ...
        public ActionResult GetFilteredItems(PurchaseMasterVM vm)
        {
            ...
            {
                ReceiveDateFrom = Convert.ToDateTime(vm.InvoiceDateTimeFrom).ToString("yyyy-MMM-dd");
            }
            if (!string.IsNullOrWhiteSpace(vm.InvoiceDateTimeTo))
            {
                ReceiveDateTo = Convert.ToDateTime(vm.InvoiceDateTimeTo).AddDays(1).ToString("yyyy-MMM-dd");
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    conditionalFields = new string[] { "pih.ReceiveDate>=", "pih.ReceiveDate<", "pih.Post", "pih.VendorID", "v.VendorGroupID", "pih.WithVDS", "pih.IsVDSCompleted isnull" };
                    conditionalValues = new string[] { ReceiveDateFrom, ReceiveDateTo, vm.Post, vendor, vm.VendorGroup, vm.WithVDS, IsVDSCompleted };
                }

            }

            var list = _repo.SelectAll(0, conditionalFields, conditionalValues, null, null, null, null, false, VDSSearch);
        ...
        public ActionResult Post(string ids)
        {
            ...
            PurchaseDetailVMS = _repo.SelectPurchaseDetail(vm.PurchaseInvoiceNo);
            TrackingVM = _repo.SelectTrakingsDetail(PurchaseDetailVMS, vm.PurchaseInvoiceNo);
        ...
        [Authorize]
        public ActionResult HSCodeUpdatePurchase(PurchaseMasterVM paramVM)
        {
            identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
            _repo = new PurchaseRepo(identity, Session);
            #region Access Control
            ...
            try
            {
                ...
                result = _repo.HSCodeUpdatePurchase(Convert.ToDateTime(paramVM.PeriodDateTime).ToString("MMMM-yyyy"));
