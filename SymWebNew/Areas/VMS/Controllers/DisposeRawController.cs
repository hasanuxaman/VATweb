...
{
    [ShampanAuthorize]
    public class DisposeRawController : Controller
    {
        //
        // GET: /Vms/FinancialTransaction/

        DisposeRawRepo _repo = null;
        ShampanIdentity identity = null;

        public DisposeRawController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new DisposeRawRepo(identity);
            }
            catch
            {
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult Index(DisposeRawsMasterVM paramVM, string transactionType)
        {
        ...
        {
            _repo = new DisposeRawRepo(identity, Session);

            List<DisposeRawsMasterVM> getAllData = new List<DisposeRawsMasterVM>();
            ...
            if (!identity.IsAdmin)
            {
                string[] conditionFields = { "TransactionDateTime>=", "TransactionDateTime<=", "Post", "BranchId" };
                string[] conditionValues = { dtFrom, dtTo, paramVM.Post, paramVM.BranchId.ToString() };
                getAllData = _repo.SelectAllWeb(0, conditionFields, conditionValues);
            }
            else
            {
                string[] conditionFields = { "TransactionDateTime>=", "TransactionDateTime<=", "Post", "BranchId" };
            ...
            IEnumerable<DisposeRawsMasterVM> filteredData;
            ...
            Func<DisposeRawsMasterVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.DisposeNo :
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult BlankItem(DisposeRawsDetailVM vm)
        {
        ...
        public ActionResult Create(string tType)
        {
            ...
            DisposeRawsMasterVM vm = new DisposeRawsMasterVM();

            List<DisposeRawsDetailVM> DisposeDetailVMs = new List<DisposeRawsDetailVM>();
        ...
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult CreateEdit(DisposeRawsMasterVM vm)
        {
            _repo = new DisposeRawRepo(identity, Session);
            string project = new System.Configuration.AppSettingsReader().GetValue("CompanyName", typeof(string)).ToString();
            if (project.ToLower() == "vms")
            ...
            try
            {
                foreach (DisposeRawsDetailVM vmD in vm.Details)
                {
                    if (vmD.SaleNo == null)
                    {
                        vmD.SaleNo = "0";
                ...
                if (vm.Operation.ToLower() == "add")
                {
                    ...
                    result = _repo.DisposeRawInsert(vm);

                    if (result[0] == "Success")
                    {
                        Session["result"] = result[0] + "~" + result[1];
                        //return RedirectToAction("Edit", new { id = result[4], TransactionType = vm.TransactionType });
                        return RedirectToAction("Edit", new { DisposeNo = result[2], TransactionType = vm.TransactionType });
                    }
                    else
                    {
                        Session["result"] = result[0] + "~" + result[1];
                ...
                else if (vm.Operation.ToLower() == "update")
                {
                    ...
                    result = _repo.DisposeRawsUpdate(vm);
        ...
        public ActionResult Edit(string id, string TransactionType, string DisposeNo)
        {
            ...
            string[] conditionFields = new string[] { "DisposeNo", "Id" };
            ...
            List<DisposeRawsDetailVM> DisposeRawDetailVMs = new List<DisposeRawsDetailVM>();

            DisposeRawDetailVMs = _repo.SelectDetail(DisposeNo);

            vm.TransactionDateTime = Convert.ToDateTime(vm.TransactionDateTime).ToString("yyyy-MMM-dd");

            vm.Details = DisposeRawDetailVMs;
        ...
        public ActionResult Post(string ids)
        {
            ...
            result = _repo.DisposeRawPost(vm);
        ...
        [Authorize]
        [HttpGet]
        public ActionResult GetItemPopUp(string targetId, string TransactionType, string ItemNo)
        {
            string project = new System.Configuration.AppSettingsReader().GetValue("CompanyName", typeof(string)).ToString();
            if (project.ToLower() == "vms")
            {
        ...
        public ActionResult GetFilteredItems(PurchaseMasterVM vm)
        {
            ...
            PurchaseRepo repo = new PurchaseRepo();
            ...
            if (vm.SearchField != null)
            {
                ...
                conditionalFields = new string[] { seachField, "pih.PurchaseInvoiceNo>", "pih.PurchaseInvoiceNo<", "pih.Post", "pih.BranchId" };
                conditionalValues = new string[] { vm.SearchValue, vm.InvoiceDateTimeFrom, vm.InvoiceDateTimeTo, vm.Post, BranchId };
            }
            else
            {
                conditionalFields = new string[] { "pih.PurchaseInvoiceNo>", "pih.PurchaseInvoiceNo<", "pih.Post", "pih.BranchId" };
                conditionalValues = new string[] { vm.InvoiceDateTimeFrom, vm.InvoiceDateTimeTo, vm.Post, BranchId };
            }
            var list = repo.SelectAll(0, conditionalFields, conditionalValues, null, null, null, vm.ItemNo, true);

            return PartialView("_filteredPurchases", list);
        }

        [Authorize(Roles = "Admin")]
        public ActionResult BlankItems(string purchaseNo, string InvoiceDate, bool SearchPreviousForCNDN = false)
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                ...
                List<DisposeRawsDetailVM> DeiposeRDetailsVms = new List<DisposeRawsDetailVM>();
                ...
                {
                    DisposeRawsDetailVM DispRawDetails = new DisposeRawsDetailVM();

                    DispRawDetails.PurchaseNo = item.PurchaseInvoiceNo;
                    DispRawDetails.Quantity = item.Quantity;
                    DispRawDetails.UnitPrice = item.UnitPrice;
                    DispRawDetails.UOM = item.UOM;
                    DispRawDetails.VATAmount = item.VATAmount;
                    DispRawDetails.VATRate = item.VATRate;
                    DispRawDetails.ItemNo = item.ItemNo;
                    DispRawDetails.Comments = item.Comments;
                    DispRawDetails.SD = item.SD;
                    DispRawDetails.SDAmount = item.SDAmount;
                    DispRawDetails.OfferUnitPrice = item.UnitPrice;
                    DispRawDetails.ProductName = item.ProductName;
                    DispRawDetails.SubTotal = item.SubTotal;


                    DeiposeRDetailsVms.Add(DispRawDetails);

                }


                return PartialView("_detailMultiple", DeiposeRDetailsVms);
            }
            catch (Exception e)
            {
