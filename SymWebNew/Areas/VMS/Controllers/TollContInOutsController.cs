...
{
    [ShampanAuthorize]
    public class TollContInOutsController : Controller
    {
        

        //
        // GET: /VMS/TollContInOuts/


        TollContInOutsRepo _repo = null;
        ShampanIdentity identity = null;

        public TollContInOutsController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new TollContInOutsRepo(identity);
            }
            catch
            {
        ...
        public ActionResult Index(TollContInOutVM paramVM)
        {
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult _index(JQueryDataTableParamVM param, TollContInOutVM paramVM)
        {
            _repo = new TollContInOutsRepo(identity, Session);

            List<TollContInOutVM> getAllData = new List<TollContInOutVM>();
            ...
            string[] conditionFields = { "DateTime>=", "DateTime<=", "TransactionType", "Code like", "Post", "TollContInOuts.BranchId", "SelectTop", "ImportIDExcel like" };
            ...
            IEnumerable<TollContInOutVM> filteredData;
            ...
            Func<TollContInOutVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.Code :
        ...
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult CreateEdit(TollContInOutVM vm)
        {
            _repo = new TollContInOutsRepo(identity, Session);
            string project = new System.Configuration.AppSettingsReader().GetValue("CompanyName", typeof(string)).ToString();
            if (project.ToLower() == "vms")
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult BlankItem(TollContInOutDetailVM vm)
        {
        ...
        public ActionResult Post(string ids)
        {
            ...
            List<TollContInOutDetailVM> TollContInOutDetailVMs = new List<TollContInOutDetailVM>();

            TollContInOutDetailVMs = _repo.SearchDetailList(vm.Id.ToString());

            vm.Details = TollContInOutDetailVMs;
