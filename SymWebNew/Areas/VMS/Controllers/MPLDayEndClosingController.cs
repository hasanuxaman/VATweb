...
{
    [ShampanAuthorize]
    public class MPLDayEndClosingController : Controller
    {
        ShampanIdentity identity = null;

        MPLDayEndClosingRepo _repo = null;

        public MPLDayEndClosingController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new MPLDayEndClosingRepo(identity);
            }
            catch
            {
        ...
        public ActionResult Index()
        {
            ...
            MPLDayEndClosingVM paramVM = new MPLDayEndClosingVM();
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult _index(JQueryDataTableParamVM param, MPLDayEndClosingVM paramVM)
        {
            _repo = new MPLDayEndClosingRepo(identity, Session);

            List<MPLDayEndClosingVM> getAllData = new List<MPLDayEndClosingVM>();
            ...
            conditionFields = new string[] { "Ded.DayEndDate>=", "Ded.DayEndDate<=", "Ded.BranchId" };
            ...
            IEnumerable<MPLDayEndClosingVM> filteredData;
            ...
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                ...
                filteredData = getAllData
                   .Where(c => isSearchable1 && c.DayEndDate.ToLower().Contains(param.sSearch.ToLower())
            ...
            Func<MPLDayEndClosingVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.DayEndDate : sortColumnIndex == 2 && isSortable_2 ? c.Code.ToString() :
        ...
        public ActionResult Create(string tType)
        {
            ...
            MPLDayEndClosingVM vm = new MPLDayEndClosingVM();
         

            vm.Operation = "add";
            vm.DayEndDate = Session["SessionDate"].ToString();
        ...
        [HttpPost]
        [Authorize]
        public ActionResult CreateEdit(MPLDayEndClosingVM vm)
        {
            try
            {
                _repo = new MPLDayEndClosingRepo(identity, Session);
                ...
                if (!string.IsNullOrEmpty(vm.DayEndDate))
                { vm.DayEndDate = Convert.ToDateTime(vm.DayEndDate).ToString("yyyy-MMM-dd"); }
                ...
                try
                {
                    ...
                    if (vm.Operation.ToLower() == "add")
                    {
                        ...
                        result = _repo.InsertToMPLDayEndClosing(vm, null, null);
                    ...
                    else if (vm.Operation.ToLower() == "update")
                    {
                        ...
                        result = _repo.UpdateMPLDayEndData(vm, null, null);
        ...
        [HttpPost]
        [Authorize]
        public ActionResult DayEndClosingPost(MPLDayEndClosingVM vm)
        {
            try
            {
                    _repo = new MPLDayEndClosingRepo(identity, Session);
                    ...
                    vm.PostedOn = DateTime.Now.ToString("dd-MMM-yyyy hh:mm:ss");
                    vm.PostedBy = identity.Name;

                    result = _repo.DayEndClosingPost(vm, null, null);
