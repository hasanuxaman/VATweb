...
{
    [ShampanAuthorize]
    public class TollClientInOutsController : Controller
    {
         //
        // GET: /VMS/TollProductionConsumption/


        TollClientInOutRepo _repo = null;
        ShampanIdentity identity = null;

        public TollClientInOutsController()
        {
            try
            {
                identity = (ShampanIdentity)Thread.CurrentPrincipal.Identity;
                _repo = new TollClientInOutRepo(identity);
            }
            catch
            {
        ...
        public ActionResult Index(TollClientInOutVM paramVM)
        {
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult _index(JQueryDataTableParamVM param, TollClientInOutVM paramVM)
        {
            _repo = new TollClientInOutRepo(identity, Session);

            List<TollClientInOutVM> getAllData = new List<TollClientInOutVM>();
            ...
            string[] conditionFields = { "DateTime>=", "DateTime<=", "TransactionType", "Code like", "Post", "TollClientInOuts.BranchId", "SelectTop", "ImportIDExcel like" };
            string[] conditionValues = { dtFrom, dtTo, paramVM.TransactionType, paramVM.Code, paramVM.Post, paramVM.BranchId.ToString(), paramVM.SelectTop, paramVM.ImportID };
            ...
            IEnumerable<TollClientInOutVM> filteredData;
            ...
            Func<TollClientInOutVM, string> orderingFunction = (c => sortColumnIndex == 1 && isSortable_1 ? c.Code :
        ...
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult CreateEdit(TollClientInOutVM vm)
        {
            _repo = new TollClientInOutRepo(identity, Session);
            string project = new System.Configuration.AppSettingsReader().GetValue("CompanyName", typeof(string)).ToString();
            if (project.ToLower() == "vms")
            ...
            try
            {
                ...
                else if (vm.Operation.ToLower() == "update")
                {
                    ...
                    ResultVM result = _repo.UpdateData(vm, identity.UserId);

                    result.Message = result.Message.Split('\r').FirstOrDefault();

                    
                    if (result.Status == "Success")
                    {
                        Session["result"] = result.Status + "~" + result.Message;
                        return RedirectToAction("Edit", new { id = result.Id, TransactionType = vm.TransactionType });
                    }
                    else
                    {
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult BlankItem(TollClientInOutDetailVM vm)
        {
            ...
            var dtbom = PRepo.SelectBOMRaw(vm.ItemNo, DateTime.Parse(vm.DateTime).ToString("yyyy-MMM-dd"));
        ...
        public ActionResult Post(string ids)
        {
            ...
            List<TollClientInOutDetailVM> TollClientInOutDetailVMs = new List<TollClientInOutDetailVM>();

            TollClientInOutDetailVMs = _repo.SearchDetailList(vm.Id.ToString());

            vm.Details = TollClientInOutDetailVMs;       
